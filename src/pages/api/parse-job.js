...
const generateInterviewQuestions = async (details) => {
  ...
  const prompt = `Write 7 short and succinct interview questions (use full sentences, not too long, that end in question marks as if you were asking these on the phone to the candidate) for the initial phone screen based on the following job posting:\n\n${details}`;
  const response = await askGPT(prompt, rule);
